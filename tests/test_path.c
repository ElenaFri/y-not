...
int main(void)
{
    ...
    /* /tmp : 2 composants */
    AccessPath *tmp = path_resolve("/tmp");
    CHECK(tmp != NULL);
    if (tmp)
    {
        CHECK(tmp->count == 2);
        CHECK(strcmp(tmp->components[0].path, "/") == 0);
        CHECK(strcmp(tmp->components[1].path, "/tmp") == 0);
    ...
    AccessPath *nope = path_resolve("/tmp/__y_not_no_such_path__");
    CHECK(nope != NULL);
    if (nope)
    {
        CHECK(nope->count == 3);
        CHECK(strcmp(nope->components[2].path, "/tmp/__y_not_no_such_path__") == 0);
